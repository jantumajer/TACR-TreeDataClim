function [trw,varargout] = VSLite_decline(syear,eyear,phi,respone_par,T,P, varargin)
...
if nargin > 6 
    ...
    hydroclim = 'P';
    Nvararg = length(varargin); # Moje modifikace
    for i = 1:Nvararg/2
...
# Loading parameters of response function
T1 = respone_par(1,1);
T2 = respone_par(1,2);
T3 = respone_par(1,3);
T4 = respone_par(1,4);
...
if phi>0 % if site is in the Northern Hemisphere:
    if I_0<0; % if we include part of the previous year in each year's modeled growth:
        ...
        startmo = I_0; # ZMENENO: Puvodne zde bylo I_0+1
        endmo = I_f;
...
elseif phi<0 % if site is in the Southern Hemisphere:
    ...
    startmo = 6+I_0; % ZMENENO: Puvodne zde bylo 7+I_0; (eg. I_0 = -3 in SH corresponds to starting integration in March of cyear)
